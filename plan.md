...
**Two texture gaps are live, and each is one sentence.** There are no mips, so a
textured floor aliases at grazing angles. And there is no colourspace but sRGB,
so a normal or roughness map through `three.texture(path)` comes back
gamma-decoded — the argument is in `Assets.load_texture_file`.
...
**There is exactly one windowed test and it is deliberate.** `test/resize_test.c3`
opens a real window and walks the swapchain through a spread of awkward sizes and
a minimise, because the bugs it exists for — the blit's destination rect, a
swapchain image destroyed while a presentation still refers to it — are only
reachable through `Renderer.frame`, and both headless frame paths fence-wait after
submitting so neither can ever see them. It skips with a printed note where no
window server exists. Anything else wanting a window should be asked why it cannot
be headless first.

...
export const DOCS = {
	...
	differences: {
		...
		'physics-world': 'There is a physics world PER SCENE, which Three.js has no equivalent of at all: object.body = { shape, mass } describes a body and three.physics.add(object) gives the object one. scene.physics is the world that belongs to a scene and three.physics is whichever scene is being rendered — so the next level\'s bodies can be built while the current one is on screen, and they stand still until you activate it, because the frame steps one world. It is XPBD with real contacts, friction, restitution, joints and triggers — not a demo. Y is down: three.physics.gravity is [0, -9.8, 0] and there is no axis to configure.',
	...
	classes: {
		Scene: {
			...
			note:
				'An independent world, made and immediately shown. It does NOT empty or free the scene '
				+ 'before it: that one keeps its objects, its bodies and its nav bake until you call '
				+ 'dispose() on it. That is what lets you build the next level while the current one is '
				+ 'still on screen — see the many-scenes topic. It is an Object3D, so moving it moves '
				+ 'everything.',
			methods: [
			...
			details: {
				isActive:
					'Whether this is the Scene being rendered. Exactly one is. A scene that is not '
					+ 'active is drawn by nothing, stepped by nothing and queried by nothing — which is '
					+ 'precisely what makes building the next level while the current one runs cost the '
					+ 'frame nothing, and also why bodies you add to it stand still until you activate it.',
				activate:
				...
				dispose:
					...
					+ 'what else holds those assets — two scenes over one kit means disposing either '
					+ 'frees nothing, which is right. Follow it with three.unloadUnused(). The Scene '
					+ 'being rendered cannot be disposed; activate another one first, because a frame '
					+ 'with no scene to draw is a black window that reads as a renderer fault.',
			},
		},
		Mesh: {
		...
		Entity: {
			...
			note:
				...
				+ 'Critter.step(name, fn) and Critter.frame(name, fn) register a system under this '
				+ 'class\'s name, so a report says Critter.walk rather than walk — the same two verbs as '
				+ 'three.systems, and system(name, fn, options) is the one with the phase left open. '
				+ 'There is NO update() to override, on purpose — continuous work is a named system in a '
			...
			methods: [
				...
				'step(name, fn, options)', 'frame(name, fn, options)', 'system(name, fn, options)',
				'pose(field, options)', 'flush()', 'sync()',
			],
		},
		Cooldown: {
		...
		NavField: {
			...
			note:
				...
				+ 'agent. Freed by dispose() and by disposing the scene it belongs to, and nothing else — a field is one float '
	...
	functions: {
		...
		'three.nav.bake(options)':
			...
			+ 'replaces the first. The bake belongs to its SCENE — scene.nav is the one that does, three.nav is '
			+ 'whichever scene is being rendered — so it survives a switch, and the next level can be baked '
			+ 'before it is shown: baking is voxelization over that scene\'s own triangles and needs no frame. '
			+ 'CHECK components ON WHAT COMES BACK: anything above 1 is a level cut into islands, '
		...
		'three.systems.step(name, fn, options) / frame / add / remove / enable / list / outline / report / clear / three.systemLoad(budgetMs)':
			...
			+ 'THE VERB IS THE CLOCK. three.systems.step(name, fn) is the fixed one — zero or more times '
			+ 'a frame at three.clock.fixedRate, the same dt every call — and it is where the rules of '
			+ 'the game go, because movement and collision drift when the step they integrate over does '
			+ 'not. three.systems.frame(name, fn) is once per drawn frame, handed what that frame was '
			+ 'actually worth, and it is where the camera, the fades and the uniform writes go. add() is '
			+ 'the same thing with { phase } left as an option, for when the phase is a variable. '
			+ 'THEY RUN IN THE ORDER YOU REGISTER THEM. For the one system in a file that has to break '
			+ 'that, name the neighbour: { before: \'fire\' } or { after: \'Player.pose\' }, one name or '
			+ 'a list, and a name that is registered further down the file is fine. { first: true } and '
			+ '{ last: true } are the two ends of the game\'s own list — the engine\'s systems still '
			+ 'bracket them, the rules drain ahead and the entity write-back behind. A loop is refused '
			+ 'where it is made; a name that matches nothing is said once and otherwise ignored. There '
			+ 'is also a numeric { order }, which is what the engine\'s own systems use and what all of '
			+ 'the above is built on, but a game should never need to reach for it. '
			+ 'outline() PRINTS THE TICK — \'step  spawn → player → foes\' on one line and \'frame '
			+ 'autopilot → fire\' on the next. Two lines because they are two lists: every step this '
			+ 'frame owes runs before any frame system, so a frame system is never between two steps '
			+ 'however the two lists read side by side. '
		...
		'three.scenes':
			...
			+ 'nothing. held says what three.sceneById(id) will give you back: true is the same Scene '
			+ 'object with its children intact, and it STAYS true across run_scripts, because the registry '
			+ 'behind it is bounded by dispose rather than by the scope that built the scene. false means '
			+ 'no Scene was ever made for it here — the scene the process starts with is the one that '
			+ 'reads that way — and sceneById mints a handle onto it instead. three.disposeInactive() '
			+ 'frees them all without naming any.',
		'three.sceneById(id)':
			'The Scene for a host id — the one a script built, or a handle onto one nothing here wrapped. '
			+ 'three.sceneById(s.id) === s for any scene this runtime built, INCLUDING one built two '
			+ 'run_scripts ago, so identity survives the end of the call that made it. For the other kind '
			+ '— the scene the process starts with — everything that goes through the host works: '
			+ 'activate, dispose, stats, export, background, raycast, pick. But children is EMPTY, because '
			+ 'those nodes are the host\'s and no object was ever made for them. It is a handle, not the '
			+ 'tree, and looking one up flips its held to true. scene.id is the other direction.',
		...
		'three.camera.attach(object, options)':
			...
			+ 'puts the eye ON the point, which is first person; { lag } is SECONDS of catch-up, 0 '
			+ 'for rigid, ~0.12 for a camera that trails. The follow runs LAST in the frame, after the '
		...
		'three.renderSize()': '{ width, height } of the offscreen image — what pick() counts in and what the returned PNG is. three.window is how big the window showing it is, and the two are free to differ.',
		'three.window': 'The window, as four things: width, height, scale and resize(width, height). width and height are DEVICE PIXELS, read off the drawable so they stay current through a live resize drag, and scale is device pixels per logical point — 1.0 on an ordinary display, 2.0 on a retina one. resize takes the same device pixels those report, so asking for a size and reading it back agrees, and it answers with whether there was a window to ask. Everything is zero and resize is false under --headless. '
			+ 'THE WINDOW IS NOT THE PICTURE: the scene renders into an offscreen target fixed at what --width/--height asked for at boot, and the window shows the whole of that stretched to fit. So a bigger window is the same pixels spread wider — softer on screen, with three.renderSize(), the PNG a screenshot returns and the coordinates scene.pick(x, y) counts in all unmoved. Resizing past the render size says so in the run\'s warnings. '
			+ 'RESIZE IS A REQUEST, not a setting. X11\'s window manager may adjust it and a Wayland compositor answers with a configure some frames later, so the new size turns up on a later frame rather than on the next line — read three.window.width back from inside the animation callback, not immediately after the call. ON WAYLAND THE SIZE NEVER READS BACK: the surface answers \'whatever the swapchain asks for\' rather than a size, so the drawable stays what the process booted at and the compositor scales it into whatever the window became. X11, macOS and Windows track it.',
	...
	'three.physics.gravity':
		'[x, y, z], y-up, read and written as an array. Set once at boot; it is a world setting and not a transform, which is why it is not a live Vector3. It is per SCENE, like the world it belongs to — scene.physics.gravity for one that is not being rendered.',
	'three.physics.count':
	...
	'three.setAnimationLoop(fn)':
			...
			+ 'It survives new three.Scene(), and so does the scene it was built for, so a callback holding '
			+ 'meshes from an older scene goes on moving them where nobody can see them — stop it yourself, '
			+ 'or re-register it after the swap. It only throws once that scene is disposed. The '
		...
		'material.dispose()':
			...
			+ 'default and line materials are shared and cannot be disposed. stats().materials is how '
			+ 'you watch it work, and the host prints a line naming the count once it passes 64.',
		...
		'three.light.shadow':
			...
			+ 'it off costs nothing and keeps the map for next time, and activating a scene — which new '
			+ 'three.Scene() does — turns it '
